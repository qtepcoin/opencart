...
class UrlTest extends OpenCartTest {
	...
	public function testProductUrl() {
		$link = $this->url->link('product/product', 'extension_id=1');
		$this->assertEquals(HTTP_SERVER . 'index.php?route=product/product&amp;extension_id=1', $link, "Could not construct product's URL");
	}

	public function testSecureProductUrl() {
		$link = $this->url->link('product/product', 'extension_id=1');
		$this->assertEquals(HTTPS_SERVER . 'index.php?route=product/product&amp;extension_id=1', $link, "Could not construct product's URL");
	}

	public function testProductUrlRewrite() {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "url_alias` SET query = 'extension_id=1', keyword = 'product-1'");
		$this->config->set('config_seo_url', 1);
		$urlAction = new Action('common/seo_url');
		$urlAction->execute($this->registry);

		$link = $this->url->link('product/product', 'extension_id=1');
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'extension_id=1'");
